...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/file_sharing_support')]]))
    await delete_messages([m, k])
    ...
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        buttons = [
            [
                ...
                InlineKeyboardButton('🧩 Join Here', url='https://t.me/Ruang_Imajinasii'),
            ]
            ]
        await m.reply("<b>You can't use this bot in this group, for that you have to make your own bot from the [SOURCE CODE](-).</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
